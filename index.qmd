...
La relación no lineal entre la penetración de la luz y la profundidad de disco de Secchi fue ya descrita por diversos autores que hallan una mejor descripción de esta como una de tipo logaritmica @modisComparison. Con las bandas B04, B05 y B06 verificamos esta relación:
...
ax[0,1].plot([min(B04),max(B04)],[max(log_SDD),min(log_SDD)], color='gray', linestyle='--')
...
ax[1,1].plot([min(B05),max(B05)],[max(log_SDD),min(log_SDD)], color='gray', linestyle='--')
...
ax[2,1].plot([min(B06),max(B06)],[max(log_SDD),min(log_SDD)], color='gray', linestyle='--')
...
Resulta sencillo generar un modelo de regresión lineal por el método de Mínimos Cuadrados Ordinarios con la librería ```sklearn``` y funciones de interés.
...
X_train, X_test, y_train, y_test = train_test_split(
  X, y, test_size=20, shuffle=True, random_state=42
)
...
El error cuadrático medio es: `{python} round(p_rmse,2)`.

El coeficiente de determinación (R²) es: `{python} round(p_r2,2)`.

:::

La correlación puede visualizarse de mejor manera utilizando la librería ``matplotlib``.
...
# Entrenamiento
ax[0].scatter(y_train, regressor.predict(X_train), color="#17A77E", alpha=0.7, label="Predicciones")
ax[0].plot([y_train.min(), y_train.max()], [y_train.min(), y_train.max()], 'k--', lw=2, label="Ideal")
ax[0].set(xlabel="Valor real", ylabel="Predicción", title="Conjunto de entrenamiento")
ax[0].legend()

# Validación
ax[1].scatter(y_test, y_pred, color="#9D50A6", alpha=0.7, label="Predicciones")
ax[1].plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], 'k--', lw=2, label="Ideal")
ax[1].set(xlabel="Valor real", ylabel="Predicción", title="Conjunto de validación")
ax[1].legend()

fig.suptitle("Regresión lineal: Valor real vs. Predicción")
plt.tight_layout()
plt.show()
```

Se obtiene un mejor resultado linearizando los valores de $SDD$ utilizando la librería `numpy`.

```{python}
import numpy as np
y = np.log(df['valor'].values)
