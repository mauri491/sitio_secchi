...
El mapa generado corresponde a la última fecha disponible.
```{python}
#| echo: false
#| classes: styled-output
import warnings
...
with rs.open(".\\recorte_acolite\\2025-06-09.tif") as raster:
            bandas = {
                'B01': raster.read(1),
                'B02': raster.read(2),
                'B03': raster.read(3),
...
plt.imshow(turb, cmap='rainbow', vmin=p5, vmax=p95)
plt.colorbar(label="SDD (cm)")
plt.title(f"Profundidad estimada 09/06/25")
plt.axis('off')
plt.tight_layout()
plt.show()
```